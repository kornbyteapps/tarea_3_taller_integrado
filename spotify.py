...
class SpotifyAPI(object):
    ...
    def base_related__artists_search(self, _id =''): #busqueda base que devuelve aritstas relacionados a otro artista
        ...
        endpoint = 'https://api.spotify.com/v1/artists/'+_id+'/related-artists' #endping + query
        r = requests.get(endpoint, headers=headers) #request
    ...
    def search(self, query=None, operator=None, operator_query=None, search_type='artist' ):#busqueda para canciones
        if query == None:
        ...
        query_params = urlencode({"q": query, "type": search_type.lower()}) #junta y codifica la query + el tipo de busqueda
